...
function toMarkdown(item: CatalogItem, secs: PopupSection[]) {
  const head = `# ${item.title}\n\n${item.popup?.intro || item.summary}\n`;
  const tags = item.tags?.length ? `\n**Tags:** ${item.tags.join(", ")}\n` : "";
  const steps = secs.map((s, i) => `\n### ${i + 1}. ${s.title}\n- ${s.items.join("\n- ")}`).join("");
