...
async function loadPromosFromCMS() {
  ...
  const xm = docs
    ...
    .map((d) => ({
      title: d.title,
      desc: d.desc ?? "",
      img: d.img ?? "https://ik.imagekit.io/pcqgvgpgi1/XMlogo.jpg",
    }));
  const lt = docs
    .filter((d) => d.group === "lt")
    .map((d) => ({
      title: d.title,
      desc: d.desc ?? "",
      img: d.img ?? "https://ik.imagekit.io/pcqgvgpgi1/LTlogo.jpg",
    }));
  return { xm, lt };
}
...
export const metadata: Metadata = {
  ...
  description: "รวมดีลพิเศษ คูปอง และโปรโมชันจาก Longtrade Academy — อัปเดตล่าสุด",
};
